...
import static io.github.daviszhao.stonemason.db.user.tables.UserTable.USER;
...
public class UserDao extends AbstractDao<UserRecord, User, Integer> {
    ...
    public List<User> fetchByUsername(String... values) {
        return fetch(USER.USERNAME, values);
    }
    ...
    public User fetchOneByUsername(String value) {
        return fetchOne(USER.USERNAME, value);
    }
    ...
    public List<User> fetchByPassword(String... values) {
        return fetch(USER.PASSWORD, values);
    }
    ...
    public User fetchOneBySalt(String value) {
        return fetchOne(USER.SALT, value);
    }
    ...
    public List<User> fetchByLocked(Boolean... values) {
        return fetch(USER.LOCKED, values);
    }
    ...
    public List<User> fetchByCreatetime(LocalDateTime... values) {
        return fetch(USER.CREATETIME, values);
    }
    ...
    public List<User> fetchByVersion(Integer... values) {
        return fetch(USER.VERSION, values);
    }

    public List<Condition> buildUserCondition(String keyword, Boolean locked) {
        List<Condition> conditionList = new ArrayList<>();
        if (hasText(keyword)) conditionList.add(USER.USERNAME.like("%" + keyword + "%"));
        if (locked != null) if (locked) conditionList.add(USER.LOCKED.isTrue());
        else conditionList.add(USER.LOCKED.isFalse());
